...
nu0 <- rep(3, 1)  # Degrees of freedom (>2 for well-defined mean)
S0 <- nu0 * rep(observation_sd, 1)  # Scale (high values imply weak prior knowledge)
m0 <- theta0
C0 <- diag(rep(2/ (p + ppx), ncol(theta)))  # Large variance for vague prior
...
for(i in 1:length(ps) ){
    ...
    if(USE_PREV){
      ...
      file_path <- paste0("/data/muscat_data/jaguir26/project1_ucsc_phd/", file_name) 
      load(file_path) 
      suffix <- paste0("_", sprintf("%.2f", p0))
      new.uts.out <- get(paste0("new.uts.out", suffix))
      new.sts.out <- get(paste0("new.sts.out", suffix))
      new.gamsig.out <- get(paste0("new.gamsig.out", suffix))
      new.theta.out <- get(paste0("new.theta.out", suffix))  
      m0 <- new.theta.out$sm0
      C0 <- new.theta.out$sC0
    }
    ...
    assign(paste0("new.uts.out", suffix), new.uts.out) 
    assign(paste0("new.sts.out", suffix), new.sts.out) 
    assign(paste0("new.gamsig.out", suffix), new.gamsig.out) 
    assign(paste0("new.theta.out", suffix), new.theta.out) 
    ...
    save(list = c(
                  ...
                  paste0("new.uts.out", suffix),
                  paste0("new.sts.out", suffix),
                  paste0("new.gamsig.out", suffix),
                  paste0("new.theta.out", suffix)     
                  ), 
