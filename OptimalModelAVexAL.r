...
Y <- array(colMeans(Y),c(1,length(colMeans(Y))))
TT <- dim(Y)[2]
J <- dim(Y)[1] - 1
#
...
# load(file_path)
# new.uts.out <- new.uts.out_50_AV
# new.sts.out <- new.sts.out_50_AV
# # new.gamsig.out <- new.gamsig.out_50_AV
# new.theta.out <-new.theta.out_50_AV
...
# lower_bounds <- c(0.9993, 0.99, 0.9993, 0.999)    
# upper_bounds <- c(0.9999, 0.9999, 0.9999, 0.9999)  # Upper bounds for each delta
lower_bounds <- c(0.999, 0.988, 0.999, 0.999998, 0.999, 0.05)   
upper_bounds <- c(0.999999, 0.999999, 0.999999, 0.999999, 0.999999, 0.65) 
...
# # d <- c(0.9991, 0.995, 0.9991, 0.999999, 0.9995, 0.6)
# d <- c(0.9991, 0.995, 0.9991, 0.999999, 0.9995, 0.6)
# ############################################

# initial_delta <- c(0.9992,  0.99,  0.9993, 0.999995,  0.999,  0.3)

initial_delta <- c( 0.9995,  # Trend
                    0.998,    # Seas year and semester
                    0.9999,  # Seas 80 month
                    0.99995, # Mem for Trans
                    0.9995,   # Cov
                    0.05)     #Trans
