...
RUNBOOK = ROOT / 'repro' / 'run' / 'HE2_EXDQLM_MULTIVAR_DROP_SHARED_RELAUNCH_PLAN_20260516.md'
...
def build_payload() -> dict[str, Any]:
    ...
    summary = {
        ...
        'blocked_publication_spec_validation': {
            'status': 'not_staged_for_drop',
            'note': 'The drop-family twin package was prepared directly from the approved shared relaunch contract rather than via a separate publication-spec-only rerun contract.',
        },
...
def _render_md(payload: dict[str, Any]) -> str:
    s = payload['summary']
    lines: list[str] = []
    lines.append('# HE2 exdqlm_multivar_drop Shared Relaunch Plan')
    lines.append('')
    lines.append('Date: 2026-05-16')
    lines.append('')
    lines.append('## Decision')
    lines.append('')
    lines.append('- family: `exdqlm_multivar_drop`')
    ...
    lines.append('- this twin package intentionally mirrors the approved shared exdqlm relaunch contract so the drop family uses the same corrected bundles, prior scale, discount block, and q50 stabilization posture as the keep family')
    lines.append('')
...
def write_outputs(out_root: Path = OUT_ROOT) -> dict[str, Any]:
    ...
    (out_root / 'HE2_EXDQLM_MULTIVAR_DROP_SHARED_RELAUNCH_PLAN_20260516.md').write_text(md, encoding='utf-8')
