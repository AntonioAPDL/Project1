#!/usr/bin/env python3
"""Build a strict day+1 synthetic GloFAS retrospective from daily cache files.

Definition:
  For each issue date c, read:
    forecast_cache/glofas/issue_date=YYYY-MM-DD/glofas_members.csv
  Keep only target_date == c + 1 day (strict one-step in daily space),
  then average across available member_* columns.

This keeps the synthetic series directly comparable to plotted daily
GloFAS forecast ensembles used by the forecats pipeline.
"""
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            ...
            "daily GloFAS forecast cache files."
        )
    )
    p.add_argument(
        "--glofas-cache-root",
        default=(
            ...
            "run_id=latest/forecast_cache/glofas"
        ),
        help="Root directory containing issue_date=*/glofas_members.csv files.",
    )
    p.add_argument(
        "--out-csv",
        default=(
            "data/glofas_synthetic_retrospective/point_series/"
            "glofas_synthetic_retro_ensemble_mean_daily.csv"
        ),
    ...
    p.add_argument(
        ...
        default=(
            "data/glofas_synthetic_retrospective/point_series/"
            "glofas_synthetic_retro_ensemble_mean_daily.meta.json"
        ),
    ...
    p.add_argument(
        ...
        default=(
            "data/glofas_synthetic_retrospective/point_series/"
            "glofas_synthetic_retro_day1_diagnostics.csv"
        ),
        help="Per-issue diagnostics CSV path.",
    )
    p.add_argument(
        "--value-col",
...
def _iter_issue_files(root: Path) -> Iterable[Path]:
    return sorted(root.glob("issue_date=*/glofas_members.csv"))


def _issue_from_path(p: Path) -> date:
    token = p.parent.name
    # token looks like issue_date=YYYY-MM-DD
...
def _process_issue_file(p: Path) -> IssueResult:
    issue = _issue_from_path(p)
    expected_target = issue + timedelta(days=1)
    try:
        df = pd.read_csv(p)
    ...
    if row.empty:
        return IssueResult(
            ...
            note="target_date == issue+1 not found",
        )
...
def main() -> int:
    ...
    root = Path(args.glofas_cache_root)
    ...
    files = list(_iter_issue_files(root))
    if not files:
        raise FileNotFoundError(
            f"No files found under {root} matching issue_date=*/glofas_members.csv"
        )

    results: List[IssueResult] = []
    for p in files:
        r = _process_issue_file(p)
        results.append(r)
        if args.verbose and r.status != "ok":
            print(
                f"[WARN] issue={r.issue_date.isoformat()} "
    ...
    diag_df = pd.DataFrame(diag_rows).sort_values(["issue_date"])
    ...
    # Resolve occasional duplicate target dates by keeping latest issue date.
    if not series_df.empty:
        ok_meta = pd.DataFrame(
            {
                "date": [r.target_date.isoformat() for r in ok_rows if r.target_date],
                "issue_date": [
                    r.issue_date.isoformat() for r in ok_rows if r.target_date
                ],
                args.value_col: [r.value for r in ok_rows if r.target_date],
            }
        ).sort_values(["date", "issue_date"])
        series_df = (
    ...
    meta = {
        "builder": "glofas_build_synthetic_retrospective_from_daily_cache.py",
        "definition": (
            "strict daily one-step synthetic: for each issue date c, use "
            "target_date=c+1 row from glofas_members.csv and average across member_*"
        ),
        "glofas_cache_root": str(root),
