...
CONFIG="${MULTIMODEL_V8_FEATURECOV_CF1_EPS_CONFIG:-$ROOT/config/multimodel_v8_featurecov_cf1_eps_sweep.template.yaml}"
LAUNCH=0
EXTRA_ARGS=()
...
BUILD_OUTPUT="$({ python3 scripts/build_multimodel_v8_featurecov_cf1_eps_matrix_configs.py --config "$CONFIG" "${EXTRA_ARGS[@]}"; } 2>&1)"
printf '%s\n' "$BUILD_OUTPUT"
...
Featurecov cf1 epsilon sweep scaffolding is ready.
No queue was launched.
Matrix dir: $MATRIX_DIR
To launch later, run:
  bash scripts/run_multimodel_v8_featurecov_cf1_eps_campaign.sh --config "$CONFIG" --launch
EOF2
  exit 0
fi
