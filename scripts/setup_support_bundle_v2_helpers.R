...
plot_usgs_png <- function(out_path, usgs_df, cutoff_date, support_start, plot_scale = "log_log1p_cms") {
  ...
  span_days <- as.numeric(max(flow_data$Date) - min(flow_data$Date))
...
plot_covariates_png <- function(out_path, covariate_df, cutoff_date, support_start) {
  ...
  span_days <- as.numeric(max(covariate_df$Date) - min(covariate_df$Date))
  x_label <- if (span_days > 3650) "%Y" else if (span_days > 730) "%Y-%m" else "%Y-%m-%d"
...
plot_retrospective_png <- function(out_path, retros_wide, cutoff_date, support_start, plot_scale = "log_log1p_cms") {
  ...
  p_g <- ggplot(df, aes(x = Date, y = GloFAS)) +
    ...
    theme(
      ...
      plot.subtitle = element_text(size = 11.5, face = "italic", hjust = 0.5),
