...
TARGET_BASE_COVARIATES = {"PPT", "SOIL", "PCA"}
...
def _flatten_spec_rows(spec_id: str, spec_cfg: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    _flatten_rows("", spec_cfg, rows, {"spec_id": spec_id})
    return rows
...
def _discover_compare_dirs(raw_paths: list[Any], cutoff: str, authoritative_compare_dir: Path | None) -> list[Path]:
    discovered: list[Path] = []
    seen: set[str] = set()

    def consider(path: Path) -> None:
        key = str(path.resolve())
        if key in seen or not path.exists() or cutoff not in path.name:
            return
        seen.add(key)
        discovered.append(path.resolve())

    if authoritative_compare_dir is not None:
        consider(authoritative_compare_dir)
...
def _extract_prior_knobs(cfg: dict[str, Any], family_cfg: dict[str, Any]) -> tuple[Any, Any]:
    model_key = str(family_cfg["model_key"])
    if model_key == "ndlm_main":
        prior_cfg = cfg.get("models", {}).get("ndlm_main", {}).get("prior", {}).get("forecast_cov", {})
        return prior_cfg.get("c_factor"), prior_cfg.get("epsilon")
    if model_key == "ndlm_univar":
        prior_cfg = cfg.get("models", {}).get("ndlm_univar", {}).get("prior", {})
        return prior_cfg.get("n0"), prior_cfg.get("S0")
    return None, None
...
def _select_best_source(
    ...
    for compare_dir in compare_dirs:
        ...
        if best is None or mean_crps < float(best["mean_crps"]):
            ...
            knob_a, knob_b = _extract_prior_knobs(cfg, family_cfg)
            best = {
                "cutoff": cutoff,
                "family_id": family_id,
                "model_id": model_id,
                ...
                "selected_knob_a": knob_a,
                "selected_knob_b": knob_b,
            }
    if best is None:
...
def _build_run_config(
    ...
    _rewrite_fit_covariates_from_source_snapshot(
        ...
        keep_names=TARGET_BASE_COVARIATES,
    )

    cfg["debug_ndlm_featurecov_rerun"] = {
        ...
        "selected_prior_knob_a": selection["selected_knob_a"],
        "selected_prior_knob_b": selection["selected_knob_b"],
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Build corrected NDLM-only feature-covariate rerun configs from authoritative cutoff source runs."
    )
...
def main() -> int:
    ...
    matrix_dir = ensure_dir(
        ...
        or (artifact_root / "control" / "ndlm_featurecov_matrix")
    )
    ...
    campaign_spec_id = str(campaign_cfg.get("spec_id", campaign_cfg.get("campaign_id", "ndlm_featurecov_v1"))).strip()
    if not campaign_spec_id:
        campaign_spec_id = "ndlm_featurecov_v1"
    ...
    enabled_families = []
    for family_id, family_cfg in _sorted_enabled(families_cfg, preferred_order=NDLM_MODEL_ORDER):
    ...
    if not enabled_cutoffs:
        raise SystemExit("No enabled cutoffs selected for NDLM featurecov rerun build.")
    if not enabled_families:
        raise SystemExit("No enabled NDLM families selected for NDLM featurecov rerun build.")
    if not enabled_specs:
        raise SystemExit("No enabled NDLM specs selected for NDLM featurecov rerun build.")
    ...
    for cutoff, cutoff_cfg in enabled_cutoffs:
        ...
        candidate_dirs = _discover_compare_dirs(
            cutoff_cfg.get("candidate_report_roots", []),
            cutoff,
        ...
        for spec_id, spec_cfg in enabled_specs:
            for family_id, family_cfg in enabled_families:
                selection = _select_best_source(cutoff, family_id, family_cfg, candidate_dirs)
                ...
                plan_rows.append(
                    {
                        ...
                        "lane": family_id,
                        "run_scope": "ndlm_featurecov_rerun",
                        "run_id": run_id,
                        ...
                        "selected_source_type": selection["source_type"],
                        "selected_source_config": selection["source_config"],
                        "selected_source_lane": selection["source_lane"],
                        "selected_mean_crps": selection["mean_crps"],
                        "selected_median_crps": selection["median_crps"],
                        "cutoff_rank": cutoff_rank[cutoff],
                    }
                )
                selection_rows.append(dict(plan_rows[-1]))
    ...
    _flatten_rows("inputs", inputs_cfg, scope_rows, {"campaign_spec_id": campaign_spec_id, "section": "inputs"})
    for family_id, family_cfg in enabled_families:
        _flatten_rows(
            "families." + family_id,
            family_cfg,
    ...
    pd.DataFrame(scope_rows).sort_values(["section", "parameter"]).reset_index(drop=True).to_csv(
        matrix_dir / "campaign_parameter_table.csv", index=False
    )
    ...
    metadata = {
        "campaign_id": str(campaign_cfg.get("campaign_id", "multimodel_v8_ndlm_featurecov_rerun")),
        "campaign_spec_id": campaign_spec_id,
        "campaign_config": str(campaign_path),
        "artifact_root": str(artifact_root),
        "matrix_dir": str(matrix_dir),
        "config_output_dir": str(config_output_dir),
        "compare_builder": str(compare_cfg.get("builder", "scripts/build_multimodel_v8_ndlm_compare_bundle.py")),
        "queue": {
    ...
    (matrix_dir / "ndlm_featurecov_scope.md").write_text("\n".join(scope_lines) + "\n", encoding="utf-8")
