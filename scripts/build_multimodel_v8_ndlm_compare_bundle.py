...
def _run_output_root(run_id: str, artifact_root: str | Path | None = None) -> Path:
...
def _load_run_tables(run_id: str, model_id: str, family_id: str, artifact_root: str | Path | None = None) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    root = _run_output_root(run_id, artifact_root)
    crps = _read_csv(root / "tables" / "crps_forecast_summary.csv")
    health = _read_csv(root / "tables" / "crps_input_health.csv")
    fig = _read_csv(root / "figure_manifest.csv", required=False)
    ...
    if crps.empty or health.empty:
        raise RuntimeError(f"Missing NDLM rows for model_id={model_id} in run_id={run_id}")
...
def _build_coverage(authoritative_coverage: pd.DataFrame, source_map: pd.DataFrame, replaced_ids: set[str]) -> pd.DataFrame:
    ...
    for _, row in source_map.iterrows():
        if str(row["model_id"]) not in replaced_ids:
            continue
        ndlm_rows.append(
            {
    ...
    out = pd.concat([rows, pd.DataFrame(ndlm_rows)], ignore_index=True)
    if not out.empty:
...
def _write_summary(
    ...
    cutoff: str,
    spec_id: str,
    authoritative_compare_dir: Path,
    source_map: pd.DataFrame,
    crps: pd.DataFrame,
) -> None:
    timestamp = datetime.now().astimezone().strftime("%Y-%m-%d %H:%M:%S %Z")
    lines = [
        f"# multimodel_{cutoff}_v8_{spec_id} compare bundle",
        "",
        f"- Generated: {timestamp}",
        f"- Cutoff: `{cutoff}`",
        f"- Spec: `{spec_id}`",
        f"- Authoritative compare source: `{authoritative_compare_dir}`",
        "- Quantile-model and ensemble rows are preserved from the authoritative compare source.",
        "- NDLM rows are replaced from the NDLM-only relaunch runs listed below.",
        "",
        "## Replaced NDLM rows",
    ]
    for _, row in source_map.sort_values(["family_id", "model_id"]).iterrows():
        lines.append(
            f"- `{row['model_id']}` <- `{row['source_run']}` ({row['family_id']}, transfer_mode=`{row['transfer_mode']}`)"
        )
...
def build_bundle(cutoff: str, spec_id: str, matrix_dir: Path, outdir: Path, artifact_root: str | Path | None = None) -> None:
    plan = pd.read_csv(matrix_dir / "matrix_plan.csv")
    cell_plan = plan.loc[(plan["cutoff"].astype(str) == str(cutoff)) & (plan["epsilon"].astype(str) == str(spec_id))].copy()
    if cell_plan.empty:
        raise RuntimeError(f"No matrix plan rows found for cutoff={cutoff} spec={spec_id}")

    authoritative_dirs = cell_plan["authoritative_compare_dir"].dropna().astype(str).unique().tolist()
    if len(authoritative_dirs) != 1:
        raise RuntimeError(f"Expected exactly one authoritative compare dir for cutoff={cutoff} spec={spec_id}; got {authoritative_dirs}")
    ...
    for _, row in cell_plan.sort_values(["family_id", "run_id"]).iterrows():
        ...
        crps, health, fig = _load_run_tables(run_id, model_id, family_id, artifact_root)
        ndlm_crps_frames.append(crps)
        ndlm_health_frames.append(health)
        if not fig.empty:
            ndlm_fig_frames.append(fig)
        source_rows.append(
            {
                "cutoff": cutoff,
                "epsilon": spec_id,
                "model_id": model_id,
                "source_run": run_id,
                "source_lane": family_id,
                "source_type": "ndlm_relaunch",
                "family_id": family_id,
    ...
    ndlm_crps = pd.concat(ndlm_crps_frames, ignore_index=True)
    ndlm_health = pd.concat(ndlm_health_frames, ignore_index=True)
    ndlm_fig = pd.concat(ndlm_fig_frames, ignore_index=True) if ndlm_fig_frames else pd.DataFrame(columns=auth_fig.columns if not auth_fig.empty else ["model_id", "plot_type", "path", "source_run", "source_lane", "source_type"])

    final_crps = _replace_rows(auth_crps, ndlm_crps)
    final_crps = _dedupe_ensembles(final_crps).sort_values(["mean_crps", "median_crps", "model_id"], kind="stable").reset_index(drop=True)
    final_health = _replace_rows(auth_health, ndlm_health).sort_values(["model_id", "source_type", "source_run"], kind="stable").reset_index(drop=True)
    ...
    if "epsilon" in final_source.columns:
        final_source["epsilon"] = str(spec_id)

    if auth_fig.empty:
        final_fig = ndlm_fig.copy()
    else:
        final_fig = _replace_rows(auth_fig, ndlm_fig).sort_values(["model_id", "plot_type", "path"], kind="stable").reset_index(drop=True)
    ...
    _write_summary(outdir, cutoff, spec_id, authoritative_compare_dir, source_map, final_crps)


def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Build an NDLM-only compare bundle by merging relaunched NDLM rows into an authoritative compare bundle.")
    ap.add_argument("--cutoff", required=True)
    ap.add_argument("--epsilon", required=True, help="NDLM spec id carried in the matrix epsilon column")
