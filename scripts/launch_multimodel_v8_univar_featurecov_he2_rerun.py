...
DEFAULT_TEMPLATE = ROOT / "config" / "multimodel_v8_univar_featurecov_he2_rerun_20260422.template.yaml"
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Build, validate, and detached-launch the HE2 univariate featurecov rerun controller."
    )
...
def main() -> int:
    ...
    if not args.skip_validate:
        subprocess.run(
            [
                "python3",
                "scripts/validate_univar_featurecov_he2_prelaunch.py",
                "--config",
                str(template_path),
            ],
            cwd=ROOT,
