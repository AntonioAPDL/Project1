...
CONFIG="${MULTIMODEL_V8_NDLM_CAMPAIGN_CONFIG:-$ROOT/config/multimodel_v8_ndlm_campaign.template.yaml}"
LAUNCH=0
EXTRA_ARGS=()
...
BUILD_OUTPUT="$({ python3 scripts/build_multimodel_v8_ndlm_matrix_configs.py --config "$CONFIG" "${EXTRA_ARGS[@]}"; } 2>&1)"
printf '%s\n' "$BUILD_OUTPUT"
...
NDLM campaign scaffolding is ready.
No queue was launched.
Matrix dir: $MATRIX_DIR
To launch later, run:
  bash scripts/run_multimodel_v8_ndlm_campaign.sh --config "$CONFIG" --launch
EOF
  exit 0
fi
