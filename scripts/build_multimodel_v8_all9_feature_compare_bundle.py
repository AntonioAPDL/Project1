...
def _load_run_tables(run_id: str, model_id: str, family_id: str, artifact_root: str | Path | None = None) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if crps.empty or health.empty:
        raise RuntimeError(f"Missing relaunch rows for model_id={model_id} in run_id={run_id}")
    ...
    for df in (crps, health, fig):
        ...
        df["source_type"] = "featurecov_relaunch"
...
def _write_summary(outdir: Path, cutoff: str, spec_id: str, authoritative_compare_dir: Path, source_map: pd.DataFrame, crps: pd.DataFrame) -> None:
    ...
    for _, row in source_map.sort_values(["family_id", "model_id"]).iterrows():
        lines.append(
            f"- `{row['model_id']}` <- `{row['source_run']}` ({row['family_id']}, source=`{row['selected_source_run']}`, c_factor=`{row.get('selected_c_factor', '')}`, epsilon=`{row.get('selected_epsilon', '')}`)"
        )
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Build an all-9 feature-covariate compare bundle by merging relaunched rows into an authoritative compare bundle.")
    ap.add_argument("--cutoff", required=True)
    ap.add_argument("--epsilon", required=True, help="Campaign spec id carried in the matrix epsilon column")
