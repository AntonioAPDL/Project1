...
def _rewrite_inputs_from_source_snapshot(
    cfg: dict[str, Any],
    *,
    source_config: str,
) -> None:
...
def _extract_spec_knobs(cfg: dict[str, Any], family_cfg: dict[str, Any]) -> tuple[Any, Any]:
    model_key = str(family_cfg["model_key"])
    if model_key == "exdqlm_multivar":
        fit_cfg = cfg.get("fit", {}).get("exdqlm_multivar", {}).get("legacy", {}).get("forecast_cov", {})
        return fit_cfg.get("c_factor"), fit_cfg.get("epsilon")
...
def _select_best_source(cutoff: str, family_id: str, family_cfg: dict[str, Any], compare_dirs: list[Path]) -> dict[str, Any]:
    ...
    for compare_dir in compare_dirs:
        ...
        if best is None or mean_crps < float(best["mean_crps"]):
            ...
            c_factor, epsilon_value = _extract_spec_knobs(cfg, family_cfg)
            best = {
                "cutoff": cutoff,
                "family_id": family_id,
                "model_id": model_id,
...
def _filter_fit_covariates(cfg: dict[str, Any], keep_names: set[str]) -> None:
    covs = cfg.get("inputs", {}).get("fit", {}).get("covariates", []) or []
    filtered = []
    for entry in covs:
    ...
    _set_nested(cfg, ["inputs", "fit", "covariates"], filtered)
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Build all-9 model relaunch configs with engineered covariates from best existing source runs.")
...
def main() -> int:
    ...
    matrix_dir = ensure_dir(_resolve_repo_path(args.matrix_dir or campaign_cfg.get("matrix_dir")) or (artifact_root / "control" / "all9_feature_matrix"))
    ...
    campaign_spec_id = str(campaign_cfg.get("spec_id", campaign_cfg.get("campaign_id", "featurecov_v1"))).strip() or "featurecov_v1"
    ...
    if not enabled_cutoffs:
        raise SystemExit("No enabled cutoffs selected for all-9 feature relaunch build.")
    if not enabled_families:
        raise SystemExit("No enabled model families selected for all-9 feature relaunch build.")
            ...
            plan_rows.append(
                {
                    ...
                    "lane": family_id,
                    "run_scope": "all9_feature_relaunch",
                    "run_id": run_id,
    ...
    for family_id, family_cfg in enabled_families:
        _flatten_rows("families." + family_id, family_cfg, scope_rows, {"campaign_spec_id": campaign_spec_id, "section": "families"})
    ...
    metadata = {
        "campaign_id": str(campaign_cfg.get("campaign_id", "multimodel_v8_all9_featurecov")),
        "campaign_spec_id": campaign_spec_id,
    ...
    (matrix_dir / "all9_feature_scope.md").write_text("\n".join(scope_lines) + "\n", encoding="utf-8")
