...
class ConfigExtremeQuantileStabilizationTests(unittest.TestCase):
    ...
    def test_negative_min_update_iters_fails_validation(self) -> None:
        with tempfile.TemporaryDirectory(prefix="ut_cfg_extreme_q_bad_min_updates_") as td:
            cfg_path = Path(td) / "cfg_bad_min_updates.yaml"
            cfg_path.write_text(
                textwrap.dedent(
                    """
                    config_version: 1
    ...
    def test_nonpositive_convergence_tol_fails_validation(self) -> None:
        with tempfile.TemporaryDirectory(prefix="ut_cfg_extreme_q_bad_tol_") as td:
            cfg_path = Path(td) / "cfg_bad_tol.yaml"
            cfg_path.write_text(
                textwrap.dedent(
                    """
                    config_version: 1
        ...
        self.assertNotEqual(proc.returncode, 0)
        self.assertIn(
            "fit.exdqlm_multivar.gamma_sigma.convergence_tol must be numeric and > 0",
            proc.stderr,
        )
