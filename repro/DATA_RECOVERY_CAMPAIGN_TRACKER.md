...
| GLOFAS operational forecasts (auxiliary) | ECMWF/CEMS forecast API | operational medium-range point cache rebuild | `operational` with issue-date interpretation into historical family windows | `river_discharge_in_the_last_24_hours` / extracted `dis24` members | 1-degree bbox GRIB download around target lat/lon plus nearest valid grid-cell extraction | prior cache window `2019-11-05` to `2023-01-31`; smoke validated on `2020-01-16` and `2022-12-25` | `glofas_operational_mediumrange_download_point.py`, `scripts/forecats_extract_glofas_batch.py`, `scripts/check_glofas_operational_forecast_health.py`, `scripts/run_glofas_operational_forecast_smoke.py`, `scripts/build_glofas_operational_split_plan.py`, `scripts/check_glofas_operational_parallel_health.py`, `scripts/run_glofas_operational_parallel_smoke.py`, `scripts/launch_glofas_operational_parallel.sh` | `ready_now` | `yes` | `no` | downloader is operational-only by design; version interpretation remains date-window based and aligns with `config/forecats_batch.site=11160500.default.yaml`; six-way parallel download campaign launched under external runtime root |
...
| GLOFAS operational forecasts (auxiliary) | two-date operational smoke (`2020-01-16`, `2022-12-25`) with extraction and rerun idempotency plus six-split orchestration smoke/launch validation | `pass` under `/data/muscat_data/jaguir26/project1_ucsc_phd_runtime/data_recovery/site=11160500/recovery_run=site11160500_recovery_20260406T185022Z/family=glofas_operational_forecasts/smoke/site11160500_glofas_operational_forecast_smoke_20260407T012316Z` and split smoke root `.../site11160500_glofas_operational_parallel_smoke_20260407T022231Z` | `in_progress` under `family=glofas_operational_forecasts/full_runs/glofas_operational_parallel_20260407T023100Z` | initial manifest health written to `health_checks/parallel_download_health.json`; split logs show all six sessions submitted accepted EWDS requests | operational forecast system version remains `operational`; follow-on extraction should use `commands/run_extract_all.sh` after the six split download sessions finish |
