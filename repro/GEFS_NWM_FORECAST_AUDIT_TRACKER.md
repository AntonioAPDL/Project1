...
| `scripts/plot_gefs_nwm_forecast_cutoff.R` | script | GEFS / NWM / cutoff plotting | newly added and usable | Reads the handoff cache for a chosen init/cutoff date and produces separate high-resolution soil and precipitation comparison figures. Supported modes now include full overlap with member spread summaries, a cleaner `mean_only` overlap mode, a covariate-overlay variant using the realized PRISM and ERA5 model covariates, a `mean_only_same_units` mode that converts series into harmonized physical units (`m3/m3` for soil, mm per forecast day for precipitation), and an optional shared-y-limit input so all cutoff plots in a run can use the same per-variable y-axis range. |
| `scripts/build_gefs_nwm_forecast_y_limit_manifest.R` | script | GEFS / NWM / plot y-limit manifest builder | newly added and usable | Scans per-cutoff plot summary JSON files and writes a reproducible `shared_y_limits.json` that can be fed back into `scripts/plot_gefs_nwm_forecast_cutoff.R` for comparable cutoff plots. |
...
| `IMPL-018` | Add shared-y-limit support so same-unit cutoff plots use comparable y ranges across all cutoff dates | `Rscript scripts/build_gefs_nwm_forecast_y_limit_manifest.R --manifest-run-dir <MANIFEST_RUN_DIR> --out-json <MANIFEST_RUN_DIR>/plots/shared_y_limits.json` plus `Rscript scripts/plot_gefs_nwm_forecast_cutoff.R --manifest-run-dir <MANIFEST_RUN_DIR> --cutoff-date 2022-12-25 --plot-style mean_only_same_units_bias_quantiles --overlay-covariates --shared-y-limits-json <MANIFEST_RUN_DIR>/plots/shared_y_limits.json` | any finished cutoff plot set | pass | Added a manifest-level shared y-limit JSON for soil and precipitation same-unit plots. The plotting script now accepts `--shared-y-limits-json`, applies the same y-range to every cutoff plot in the run, and records the applied ranges back into each `plot_summary*.json` for reproducibility. |
