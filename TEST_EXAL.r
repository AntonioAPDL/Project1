...
initial_delta   <- c(0.9995, 0.9995, 0.9995, 0.9995 , 0.9995   , 0.94)
delta = initial_delta
SIMS = TRUE
use_covariates = TRUE
...
lam1 <- 0.9999 # Sudden correction at start of forecast period
lam2 <- 0.9999 # Correction during forecast period from historical period
...
###########################################################
DDD1 <- X[,1]-min(X[,1]); DDD2 <- X[,2]-min(X[,2]); DDD3 <- X[,3]-min(X[,3]);
x1 <- (exp(DDD1)); x1 <- x1;
x2 <- ((DDD2)); x2 <- x2;
x3 <- ((DDD3)); x3 <- x3;
X <- cbind(x1,x2,x3)
# TT <- dim(as.matrix(plot_data[, c('USGS', 'GloFAS', 'NWS3.0')]))[1]
# x4 <- rep(1,TT); 
# X <- cbind(x1,x2,x3,x4)
###########################################################
...
df.mat_f1 <- make_df_mat(df*lam1, dim.df, p)
df.mat.k_f1 <- make_df_mat_k(df*lam1, dim.df, p, k)
...
while (FLAG & iter < max_iter) {
 ...
 if ((crit_ELBO+conv.check) < tol1 || iter < max_iter) {
  # if ((crit_ELBO+conv.check) < tol1 || iter < 100 || fast > 0 ) {
    update.theta <- DISC_update_theta_synth_cpp( GG, m0, C0, 
                                            FFF, QQQ, 
