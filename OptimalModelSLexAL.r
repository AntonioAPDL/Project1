...
UNIV_SHARED_INPUT_ROOT <- Sys.getenv("UNIV_SHARED_INPUT_ROOT", "")
if (nzchar(UNIV_SHARED_INPUT_ROOT)) {
  UNIV_SHARED_INPUT_ROOT <- normalizePath(UNIV_SHARED_INPUT_ROOT, mustWork = FALSE)
}
UNIV_COVARIATES_DIR <- Sys.getenv("UNIV_COVARIATES_DIR", "")
if (!nzchar(UNIV_COVARIATES_DIR) && nzchar(UNIV_SHARED_INPUT_ROOT)) {
  UNIV_COVARIATES_DIR <- file.path(UNIV_SHARED_INPUT_ROOT, "covariates")
}
if (nzchar(UNIV_COVARIATES_DIR)) {
...
glofas_forecast <- read.csv(resolve_shared_input_path(
  env_key = "UNIV_GLOFAS_FORECAST_CSV",
  ...
  label = "UNIV GloFAS forecast CSV",
  shared_root = UNIV_SHARED_INPUT_ROOT
))
...
if (!exists("family_shared_build_featurecov_design_matrices", inherits = TRUE)) {
  stop("family_shared_build_featurecov_design_matrices is unavailable; could not load shared input helpers", call. = FALSE)
}
...
X <- design$X
X_f <- design$X_f
cat(sprintf("[univar_legacy] covariate_mode=%s covariate_cols=%d\n", design$mode, ncol(X)))
flush.console()
...
if(USE_PREV){
  ...
  if (!nzchar(prev_override)) {
    ...
    prev_override <- sprintf("/data/muscat_data/jaguir26/project1_ucsc_phd/variables_%s_exAL_synth_DISC_uni.RData", result_suffix_prev)
  }
  file_path <- require_readable_path(prev_override, "UNIV previous-state RData")
  load(file_path)
  result_suffix_prev <- sprintf("%.0f", p0 * 100)
  ending_prev <- "_exAL_synth_DISC_uni"
  new.uts.out <- get(paste0("new.uts.out_", result_suffix_prev, ending_prev))
  new.sts.out <- get(paste0("new.sts.out_", result_suffix_prev, ending_prev))
  new.gamsig.out <- get(paste0("new.gamsig.out_", result_suffix_prev, ending_prev))
  new.theta.out <- get(paste0("new.theta.out_", result_suffix_prev, ending_prev))
...
save_variables <- function(var_names, filename, dir_path) {
  ...
  out <- Sys.getenv("UNIFIED_UNIV_RDATA_OUT", "")
