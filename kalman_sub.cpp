...
// [[Rcpp::export]]
List update_theta_cpp_sub(const arma::cube& GG, const arma::cube& GG_filt, const arma::cube& GG_smth, 
    ...
    for (int t = 1; t < TT; ++t) {
        ...
        a = GG_filt.slice(t) * m.col(t-1);
        P = GG_filt.slice(t) * C.slice(t-1) * GG_filt.slice(t).t(); 
        R = P % arma::pow((ex_df_mat + Ones), dM);  
...
// CHANGE THIS
    ...
    for (int t = TT-2; t >= 0; --t) {
        ...
        arma::vec a = GG_filt.slice(t+1) * m.col(t);
        arma::mat P = GG_filt.slice(t+1)* C.slice(t) * GG_filt.slice(t+1).t();
        arma::mat R = P % arma::pow((ex_df_mat + Ones), dM);  
        R = (R + R.t()) / 2;
        ...
        arma::mat sB = C.slice(t) * GG_smth.slice(t).t() * R_inv;
        ...
        // a = GG.slice(t+1) * m.col(t); I already computed this
