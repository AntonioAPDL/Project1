...
disc_w_resolve_paths <- function() {
  ...
  retros_cutoff_default <- sprintf(
    "/data/muscat_data/jaguir26/project1_ucsc_phd/retros_%s.csv",
    format(cutoff_date, "%Y-%m-%d")
  )
  retros_legacy_default <- "/data/muscat_data/jaguir26/project1_ucsc_phd/retros_2022-12-25.csv"
  retros_default <- if (file.exists(retros_cutoff_default)) retros_cutoff_default else retros_legacy_default
  ...
  paths <- list(
    ...
    retros_path = env_or_default("DISC_W_RETROS_PATH", retros_default),
