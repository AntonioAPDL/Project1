...
objective_deltas <- function(delta){

df_t    <- delta[1]
df_s    <-  delta[2]
df_s67  <- delta[3]
df_discrep <- rep(delta[4], 3-1)
...
n <- 200
for(k in 1:n){
errors = new.theta.out$standard_forecast_errors[1,]
T_e <- length(errors)
...
lower_bounds <- c(0.998, 0.998, 0.998, 0.95)    # Lower bounds for each delta
upper_bounds <- c(0.9999, 0.9999, 0.9999, 0.9999) # Upper bounds for each delta
initial_delta <- upper_bounds*0.01 + lower_bounds*0.99

opts <- list("algorithm"="NLOPT_LN_BOBYQA",    # Using a derivative-free algorithm suitable for bound constraints
             "xtol_rel"=1.0e-8,
             "maxeval"=1000)

# Wrapper to convert maximization to minimization
objective_deltas_min <- function(delta) {
  -objective_deltas(delta)  # Minimize the negative of the original function
}

# Use the wrapper function in the optimizer
result <- nloptr(x0 = initial_delta,
                 eval_f = objective_deltas_min,  # Pass the correct function here
                 lb = lower_bounds,
                 ub = upper_bounds,
                 opts = opts)
