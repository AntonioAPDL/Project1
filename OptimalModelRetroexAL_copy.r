...
objective_deltas <- function(delta, SIMS){
...
# Assuming Y, seq.gamma, seq.sigma, idx, and new.theta.out, new.sts.out, new.uts.out are defined correctly
TT <- dim(Y)[2]
...
process_time_series <- function(data, idx, TT) {
  result <- t(apply(data, 1, function(row) {
    row_subset <- row[1:length(idx)]
    interpolate_spline(idx, TT, row_subset)
  }))
  
  if (J > 0) {
    result[, last_idx] <- rowMeans(result[, 1:idx[length(idx)]])
  } else {
    result[, last_idx] <- mean(result[, 1:idx[length(idx)]])
  }
...
# Save the variables
save_variables(vars_to_save, paste0("variables_", result_suffix, ending,".RData"), "/home/jaguir26/projects/notebooks")
...
lower_bounds <- c(0.9985, 0.985, 0.9985, 0.91)    
