...
def test_master_workflow_audit_tracker_current_state() -> None:
    ...
    assert summary['publication_parity_gate']['promoted_rows'] == 15
    assert summary['publication_parity_gate']['pending_rows'] == 30
    assert summary['publication_parity_gate']['remaining_model_families_pending'] == 6
    ...
    promoted = [r for r in rows if r['label'] in {'exAL-M-T1', 'AL-M-T1', 'exAL-M-T0'}]
    ...
    pending = [r for r in rows if r['label'] not in {'exAL-M-T1', 'AL-M-T1', 'exAL-M-T0'}]
