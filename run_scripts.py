...
def run_in_tmux(session_name, script_path, param):
    ...
    result = subprocess.run(['tmux', 'send-keys', '-t', session_name, command, 'C-m'], capture_output=True, text=True)
    if result.returncode != 0:
        print(f"Error running command for session {session_name} with param {param}: {result.stderr}")

if __name__ == "__main__":
    script_path = "/home/jaguir26/project1_ucsc_phd/Optimal_DQLM.r"
    param_list = [0.05, 0.2, 0.35, 0.5, 0.65, 0.8, 0.95]

    for param in param_list:
        session_name = f"session_fit_{int(param * 100):02d}"
