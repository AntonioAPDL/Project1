...
Status as of 2026-06-12: **prepared but not launch-ready**.

The config builder and unit tests pass, and the frozen manifest matches the
requested override table for all overridden model rows. However, prelaunch
validation correctly blocked the launch on the `20210123` / `exAL-M-T0` /
`q05` smoke lane. A targeted 100-iteration diagnostic confirmed this is not a
10-iteration smoke artifact:
...
python3 scripts/launch_he2_bayesian_publication_relaunch.py \
  --template config/he2_bayesian_publication_relaunch_table1_targeted_repair_20260612.template.yaml \
  --start-monitor \
  --monitor-out-dir reports/he2_table1_targeted_repair_20260612_live \
